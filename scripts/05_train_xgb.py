...
def train(data_dir: str, out_dir: str, max_iter: int, random_state: int) -> None:
    ...
    joblib.dump(model, f"{out_dir}/model_xgb.joblib")
    with open(f"{out_dir}/metrics_xgb.json", "w") as f:
        json.dump(metrics, f, indent=2)

    print(f"[Training] Model {out_dir}/model_xgb.joblib")
    print(f"[Training] Metrics {out_dir}/metrics_xgb.json")
